...
def forum_form_discussion(request, course_key):
    ...
    if request.is_ajax():
        ...
        context = {
            ...
            'can_create_comment': _attr_safe_json(cached_has_permission(request.user, "create_comment", course.id)),
            'can_create_subcomment': _attr_safe_json(cached_has_permission(request.user, "create_sub_comment", course.id)),

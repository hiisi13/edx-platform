...
class GroupConfiguration(object):
    ...
    def get_content_groups_usage_info(store, course):
        ...
        """
        content_groups = store.get_items(course.id, settings={'group_access': {'$exists': True}})

        return GroupConfiguration._get_content_groups_usage_info(store, course, content_groups)

    @staticmethod
    def _get_content_groups_usage_info(store, course, content_groups):
        """
        Returns all units names, their urls and validation messages.
        ...
        for group in content_groups:
            if hasattr(group, 'group_access'):
                ...
                for group_id in group_ids:
                    ...
                    unit_location = store.get_parent_location(group.location)
                    if not unit_location:
                        log.warning("Parent location of content_groups module not found: %s", group.location)
                        continue
                    ...
                    # validation_summary = group.general_validation_message()
                    usage_info[group_id].append({
                        'label': '{} / {}'.format(unit.display_name, group.display_name),
                        'url': unit_url,
                        # 'validation': validation_summary.to_json() if validation_summary else None,
                    })
        return usage_info
    ...
    def update_usage_info(store, course, configuration):
        ...
        if configuration.scheme == 'random':
    ...
    def update_content_group_usage_info(store, course, configuration):
        ...
        for group in content_group_configuration['groups']:
            ...
            groups.append(group)
        content_group_configuration['groups'] = groups
    ...
    def remove_content_group_from_configuration(configuration, group_id):
        matching_group = [group for group in configuration.groups if group.id == group_id]
        group = matching_group[0] if matching_group else None
...
def group_configurations_detail_handler(request, course_key_string, group_configuration_id, group_id=0):
    """
